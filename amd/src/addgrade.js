    ...
    function($, Config, Str, ModalFactory, ModalEvents, Fragment, Ajax, Swal, Y) {
        ...
        AddGrade.prototype.registerEventListeners = function() {
            $(".grade-portfolio").click(function(event) {
                ...
                this.openModal(this.gradebutton.data('courseid'), this.gradebutton.data('groupid'));
            }.bind(this));
        };

        AddGrade.prototype.openModal = function(courseid, groupid) {
            ModalFactory.create({
                type: ModalFactory.types.SAVE_CANCEL,
                title: 'Add grade',
                body: this.getBody({courseid: courseid, groupid: groupid}),
            }).then(function(modal) {
                this.modal = modal;

                // We want to reset the form every time it is opened.
                this.modal.getRoot().on(ModalEvents.hidden, function() {
                    this.modal.setBody(this.getBody({courseid: courseid, groupid: groupid}));
                }.bind(this));
